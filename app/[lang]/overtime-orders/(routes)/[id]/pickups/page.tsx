...
export default async function ProductionOvertimePage(props: {
  ...
  if (!session) {
    redirect(
      `/${lang}/auth?callbackUrl=/${lang}/overtime-orders/${id}/pickups`,
    );
  }
  ...
  const { overtimeRequestLocaleString: request } = await getOvertimeRequest(
    lang,
    id,
  );
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <CardDescription>ID zlecenia: {request.internalId}</CardDescription>
          </div>
      ...
      <DataTable
        data={(request.employeesWithScheduledDayOff || []).map((employee) => ({
          ...employee,
