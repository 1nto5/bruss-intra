...
async function getInventoryItems(searchParams: URLSearchParams) {
  ...
  if (
    searchParams.has("purchaseDateFrom") ||
    searchParams.has("purchaseDateTo")
  ) {
    query.purchaseDate = {};
  ...
  if (
    searchParams.has("assignmentDateFrom") ||
    searchParams.has("assignmentDateTo")
  ) {
  ...
  try {
    ...
    return {
      items: items as unknown as ITInventoryItem[],
      fetchTime: new Date(),
    };
  } catch (error) {
