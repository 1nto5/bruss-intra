...
import type { I18nString, CompetencyRating, RequiredCompetency, CompetencyType } from './lib/types';
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>{dict.myProfile.notFoundDescription}</CardDescription>
        </CardHeader>
      </Card>
    );
  }
        ...
        return (
          <Card className="border-amber-300 bg-amber-50 dark:border-amber-700 dark:bg-amber-950/30">
            <CardHeader>
              ...
              <CardDescription>
                {formatDate((pendingEval as Record<string, unknown>).periodFrom as string | Date)} -{' '}
                {formatDate((pendingEval as Record<string, unknown>).periodTo as string | Date)}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Link href={`/${lang}/competency-matrix/evaluations/${((pendingEval as Record<string, unknown>)._id as string).toString()}/self`}>
                <Badge variant="statusPending" className="cursor-pointer text-sm px-3 py-1">
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex flex-wrap items-center gap-4 text-sm">
                <div>
                  <span className="text-muted-foreground">{dict.evaluations.period}: </span>
                  {formatDate(ev.periodFrom as string | Date)} - {formatDate(ev.periodTo as string | Date)}
                </div>
                <div>
                  <span className="text-muted-foreground">{dict.evaluations.assessor}: </span>
