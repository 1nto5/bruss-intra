...
const TableCellCorrectiveAction: React.FC<TableCellCorrectiveActionProps> = ({
  ...
  const confirmActionExecutionSchema = createConfirmActionExecutionSchema(
  ...
  const getStatusBadge = (statusValue: string) => {
    switch (statusValue) {
      case "open":
        return (
          <Badge variant="statusOpen">
            {dict.view.correctiveActionStatuses.open}
          </Badge>
        );
      case "closed":
        return (
          <Badge variant="statusApproved">
            {dict.view.correctiveActionStatuses.closed}
          </Badge>
        );
      case "overdue":
        return (
          <Badge variant="statusOverdue">
            {dict.view.correctiveActionStatuses.overdue}
          </Badge>
        );
      case "in progress":
        return (
          <Badge variant="statusInProgress">
            {dict.view.correctiveActionStatuses.inProgress}
          </Badge>
        );
      case "rejected":
        return (
          <Badge variant="statusRejected">
            {dict.view.correctiveActionStatuses.rejected}
          </Badge>
        );
      default:
        return (
  ...
  const onSubmit = async (
    ...
    toast.promise(
      new Promise<void>(async (resolve, reject) => {
        try {
          ...
          if (res.success) {
            ...
            reject(
              new Error(dict.view.correctiveActionDialogs.errors.contactIT),
            );
          }
        } catch (error) {
      ...
      }),
      {
        ...
        error: (err) =>
          err.message || dict.view.correctiveActionDialogs.errors.contactIT,
      },
    );
  };
  ...
  return (
    <>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            {correctiveAction.status.value === "closed" ? (
              <DialogHeader>
                <DialogTitle>
                  {dict.view.correctiveActionDialogs.closedTitle}
                </DialogTitle>
                <DialogDescription>
              ...
              <>
                <DialogHeader>
                  <DialogTitle>
                    {dict.view.correctiveActionDialogs.changeStatusTitle}
                  </DialogTitle>
                </DialogHeader>
                <Form {...form}>
                    ...
                    <div className="grid gap-2">
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem className="flex flex-col">
                            <FormLabel>
                              {dict.view.correctiveActionDialogs.changeDate}
                            </FormLabel>
                            <FormControl>
                              <DateTimePicker
                                modal
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>
                              {dict.view.correctiveActionDialogs.status}
                            </FormLabel>
                            <Select
                            ...
                            >
                              <FormControl>
                                <SelectTrigger>
                                  <SelectValue
                                    placeholder={
                                      dict.view.correctiveActionDialogs
                                        .selectPlaceholder
                                    }
                                  />
                                </SelectTrigger>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>
                              {dict.view.correctiveActionDialogs.additionalInfo}
                            </FormLabel>
                            <FormControl>
                              <Textarea
                                placeholder={
              ...
              <DialogHeader>
                <DialogTitle>
                  {dict.view.correctiveActionDialogs.noPermissionTitle}
                </DialogTitle>
                <DialogDescription>
      ...
      <TableCell>{formatDate(correctiveAction.deadline)}</TableCell>
      ...
      <TableCell>
        <Dialog>
          ...
          <DialogContent className="sm:max-w-[768px]">
            <DialogHeader>
              <DialogTitle>
                {dict.view.correctiveActionDialogs.history}
              </DialogTitle>
            </DialogHeader>
            ...
            <div className="h-[300px] overflow-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead>
                      {dict.view.correctiveActionDialogs.historyColumns.status}
                    </TableHead>
                    <TableHead>
                      {dict.view.correctiveActionDialogs.historyColumns.date}
                    </TableHead>
                    <TableHead>
                      {dict.view.correctiveActionDialogs.historyColumns.comment}
                    </TableHead>
                    <TableHead>
                      {dict.view.correctiveActionDialogs.historyColumns.person}
                    </TableHead>
                    <TableHead>
                      {dict.view.correctiveActionDialogs.historyColumns.time}
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
