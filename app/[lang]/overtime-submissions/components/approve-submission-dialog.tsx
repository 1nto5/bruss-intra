...
export default function ApproveSubmissionDialog({
  ...
  const hasQuotaInfo =
    submissionHours !== undefined && remainingQuota !== undefined;
  ...
  if (isFinalApproval && hasQuotaInfo) {
    ...
    description = (dict.dialogs.approve.descriptionPayment ?? "")
      ...
      .replace(
        "{remaining}",
        String(Math.max(0, remainingQuota - submissionHours)),
      );
    ...
    description = (dict.dialogs.approve.descriptionEscalate ?? "").replace(
      "{hours}",
