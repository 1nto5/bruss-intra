...
import { redirectToCardPositions } from "../../../../actions/utils";
...
export default function EditPositionForm({
  ...
  const [pendingAction, setPendingAction] = useState<"save" | "approve" | null>(
    null,
  );
  const [isPendingFindBin, setIsPendingFindBin] = useState(false);
  const [foundBins, setFoundBins] = useState<
  ...
  const performBinSearch = useCallback(
      ...
      try {
        ...
        if ("error" in res) {
          switch (res.error) {
            ...
            case "too many bins":
              setFindBinMessage(
                dict.editPage.tooManyBins || "Too many results - refine search",
              );
              setFoundBins([]);
              break;
            default:
  ...
  const onSubmit = async (data: z.infer<typeof updatePositionSchema>) => {
    try {
      ...
      if (res.success) {
        ...
        form.setError("bin", {
          message:
            dict.editPage.binRequired || "BIN is required for this warehouse",
        });
      } else if (res.error === "bin not allowed") {
        form.setError("bin", {
          message:
            dict.editPage.binNotAllowed ||
            "BIN is not allowed for this warehouse",
        });
        ...
        <div>
          ...
          {position.approver && position.approvedAt && (
            <CardDescription className="mt-1">
              ...
              {new Date(position.approvedAt).toLocaleTimeString("pl", {
                hour: "2-digit",
                minute: "2-digit",
              })}{" "}
              {dict.editPage.by} {extractNameFromEmail(position.approver)}
            </CardDescription>
          )}
        </div>
      </CardHeader>
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                  <div className="space-y-0.5">
                    ...
                    {position.approver && (
                      <FormDescription>
                        {dict.positions.approver}:{" "}
                        {extractNameFromEmail(position.approver)}
                      </FormDescription>
                    )}
                  </div>
                  <FormControl>
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder={
                            dict.editPage.searchBinPlaceholder ||
                            "Search bin..."
                          }
                          onChange={(e) => {
                            handleFindBin(e);
